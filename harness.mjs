...
function snapshotSummary(snapshot) {
  return {
    ...
    activeTurnIndex: snapshot.context.activeTurnIndex,
    selectedTurnIndex: snapshot.context.selectedTurnIndex,
...
function runHarness() {
  ...
  send({ type: 'SELECT_TURN', turnIndex: 2 }, 'switch to Goblin 2 turn');
  check('select turn enters drafting without toggling panel', (s) => (
    String(s.value) === 'drafting'
    && s.context.selectedTurnIndex === 2
    && s.context.recordingPanel.open
  ));

  send({ type: 'SET_ACTION', action: 'raw' }, 'use raw shorthand mode');
  send({ type: 'INPUT_CHANGED', field: 'rawShorthand', value: 'cast:shield/PC_A' }, 'type shorthand');
  check('draft preview reflects raw shorthand', (s) => s.context.draft.preview === 'cast:shield/PC_A');
  ...
  send({ type: 'SELECT_TURN', turnIndex: 1 }, 'select Goblin 1');
  send({ type: 'SET_ACTION', action: 'raw' }, 'raw mode again');
  send({ type: 'INPUT_CHANGED', field: 'rawShorthand', value: 'atk:PC_A++4/dmg=7' }, 'enter attack shorthand');
  send({ type: 'CONFIRM' }, 'commit turn');
  check('confirm commits and keeps panel open', (s) => {
    const key = `${s.context.round}:1`;
    return (
      String(s.value) === 'recording'
      && s.context.turnEntries[key] === 'atk:PC_A++4/dmg=7'
      && s.context.recordingPanel.open
    );
  });
  ...
  check('next turn advances selected and active indexes', (s) => (
    String(s.value) === 'recording'
    && s.context.activeTurnIndex === 1
    && s.context.selectedTurnIndex === 1
  ));
