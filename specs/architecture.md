...
Every persisted object SHALL have one globally unique opaque internal ID. UUIDs
are the default representation. IDs must be stable across saves, reloads, and
JSON export/import, must never be reused after deletion, and must not be shown
to the user by default. Where an object may be referenced outside its ancestry,
its global ID SHALL remain stable and the reference SHALL be represented
explicitly.
...
1. labels in the target encounter;
2. labels in the wider campaign data; and
